...
############################################################################
# turned into a package for the YaoWiki project.
# - Changed package names to not interfere with originals
# - Created subroutine out of main code
# - replaced all print statements with assignments to @Output variable.
############################################################################
package DiffCmd;
use strict;
use DiffAlgor qw(diff);
...
# Contains the output of the diffcmd.
my @Output=();
...
# diffcmd
#
# Usage: @results = diffcmd($cmd,$oldfile,$newfile);
# Where $cmd = "-c"|"-Cnlines"|"-e"|"-f" |"-u"|"-Unlines"|"-q"|""
#    -c do a context diff with 3 lines of context
#    -C do a context diff with 'nlines' lines of context (implies -c)
#    -u do a unified diff with 3 lines of context
#    -U do a unified diff with 'nlines' lines of context (implies -u)
#    -q report only whether or not the files differ
#    -qf same as above but with newfile actually containing just a text string.
#    -o default "old-style" diff, with output like UNIX diff
#
#-----------------------------------------------------------------------------
sub diffcmd
{
  ...
  my ($opt_c, $opt_u, $opt_e, $opt_f, $opt_q);
    ...
    if ($opt =~ /^-C(.*)/) {
      ...
      return ("Error:diffcmd: incorrect cmd"); # error;
    }

  ######## DO THE DIFF!
  my ($file1, $file2) = ($oldfile,$newfile);
  ...
  if(!open(F1,'<',$file1)) { return ("Error:diffcmd: Couldn't open F1:$file1");}
  chomp(@f1 = <F1>);
  close F1;
  my $line;

  if($Diff_Type ne "SPECIAL") {
    if(!open(F2,'<',$file2)) { return ("Error:diffcmd: Couldn't open F2:$file2");}
    chomp(@f2 = <F2>);
    close F2;
  }
  else # file2 is actually a textstring containing the whole file. Put it into an array...
  {
    @f2 = split(/\n/,$file2);
    foreach $line (@f2) {
  ...
  #zzz REMOVE the Revision Record for comparison as they will always be different.
  foreach $line (@f1) {
    $line =~ s/<!--REVISION_TIMESTAMP.*?-->//g;
  }
  if($Diff_Type ne "SPECIAL") {
  foreach $line (@f2) {
    $line =~ s/<!--REVISION_TIMESTAMP.*?-->//g;
  ...
  my $diffs = DiffAlgor::diff(\@f1, \@f2);
  if(!@$diffs) { return @Output; }

  if ($opt_q) {
    if(@$diffs)
    {
      push @Output,"Files are different";
    }
    else
    {
       @Output=();
  ...
  if ($Diff_Type =~ /UNIFIED|CONTEXT/) {
     my @st = stat($file1);
     my $MTIME = 9;
     push @Output, ("$char1 $file1\t", scalar localtime($st[$MTIME]), "\n");
     @st = stat($file2);
      push @Output, ( "$char2 $file2\t", scalar localtime($st[$MTIME]), "\n");
  }
...
# there's only one block in the hunk.
    ...
    push @Output, "$range1$action$range2\n";
...
# last arg is type of diff
    ...
    my $action = $op_hash{$op} || warn "unknown op $op";
    my $ttemp = ($diff_type eq "ED" ? "$range1$action\n" : "$action$range1\n");
    push @Output, $ttemp;
